...
{% hint style="info" %}
This is the **self-hosted RPC endpoint** exposed by every Stacks node. It does not require [API keys](../api-keys.md) and is not subject to Hiro's [rate limits](../rate-limits.md). The Hiro-hosted Stacks Blockchain API, by contrast, is a managed service that proxies and extends these RPC endpoints with additional indexing, caching, and query capabilities.
{% endhint %}
