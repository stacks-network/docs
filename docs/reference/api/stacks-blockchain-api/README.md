...
The Stacks Blockchain API is the recommended API for wallets, dApps, explorers, and general developers who need rich query capabilities without running their own node. It proxies all standard Stacks Node RPC endpoints and adds the `/extended/` family of indexed endpoints on top.

If you need a more decentralized setup or want direct access to the raw node RPC without a third-party service, see the Stacks Node RPC API. For exchange or institutional integrations using the Coinbase Mesh standard, see the Stacks Mesh API.
...
For more usage examples, see Usage.
