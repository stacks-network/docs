...
| `(buff max-len)`                                      | The `buff` type in Clarity is a fixed-length byte sequence. You define it with a max length like `(buff 32)`, which represents a byte buffer of up to 32 bytes. Max `buff` length is 1,048,576 bytes |
